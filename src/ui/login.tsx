  ...
  return Layout({
    ...
    children: html`
      <div class="min-h-screen flex items-center justify-center p-4">
        ...
        <div
          class="fixed top-0 left-1/2 -translate-x-1/2 w-[600px] h-[400px] bg-accent-cyan/5 rounded-full blur-[120px] pointer-events-none"
        >
        </div>
        ...
        <div class="w-full max-w-md" x-cloak x-data="loginApp()">
          ...
          <template x-if="autoLogin">
            <div class="flex flex-col items-center gap-4 animate-in">
              <div
                class="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-surface-700 glow-border"
              >
                <svg
                  class="w-8 h-8 text-accent-cyan"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  stroke-width="2"
              ...
              <div class="flex items-center gap-2.5 text-gray-400">
                <svg
                  class="animate-spin h-4 w-4 text-accent-cyan/60"
                  viewBox="0 0 24 24"
                >
                  <circle
                    cx="12"
          ...
          <template x-if="!autoLogin">
            <div>
              <div class="text-center mb-8 animate-in">
                <div
                  class="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-surface-700 glow-border mb-6"
                >
                  <svg
                    class="w-8 h-8 text-accent-cyan"
                    viewBox="0 0 24 24"
              ...
              <div class="glass-card p-8 glow-cyan animate-in delay-1">
                <p class="text-xs text-gray-500 mb-6 leading-relaxed">
                  Log in with the <span class="text-gray-400">ADMIN_KEY</span> for
                  full dashboard access, or any <span class="text-gray-400"
                  >API key</span> for limited access.
                </p>

                <form @submit.prevent="login()" class="space-y-5">
                  <div>
                    <label
                      class="block text-xs font-medium text-gray-400 mb-2 uppercase tracking-widest"
                    >Key</label>
                    <input
                      type="password"
                      x-model="authKey"
                ...
                <div
                  x-show="error"
                  x-transition
                  class="mt-4 p-3 rounded-lg bg-accent-rose/10 border border-accent-rose/20 text-accent-rose text-sm"
                >
